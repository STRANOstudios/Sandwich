...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        Rect GetSafeArea()
        {
            ...
            if (InvertSafeArea)
            {
                float safeLeft = safeArea.x;
                float safeRight = safeArea.x + safeArea.width;
                float safeTop = safeArea.y;
                float safeBottom = safeArea.y + safeArea.height;
                ...
                {
                    if (safeTop > 0)
                        return new Rect(0, 0, Screen.width, safeTop);

                    if (safeBottom < Screen.height)
                        return new Rect(0, safeBottom, Screen.width, Screen.height - safeBottom);

                    if (safeLeft > 0)
                        return new Rect(0, safeTop, safeLeft, safeBottom - safeTop);

                    if (safeRight < Screen.width)
                        return new Rect(safeRight, safeTop, Screen.width - safeRight, safeBottom - safeTop);
                }
                else
                {
                    if (safeLeft > 0)
                        return new Rect(0, 0, safeLeft, Screen.height);

                    if (safeRight < Screen.width)
                        return new Rect(safeRight, 0, Screen.width - safeRight, Screen.height);

                    if (safeTop > 0)
                        return new Rect(safeLeft, 0, safeRight - safeLeft, safeTop);

                    if (safeBottom < Screen.height)
                        return new Rect(safeLeft, safeBottom, safeRight - safeLeft, Screen.height - safeBottom);
                }
            }
